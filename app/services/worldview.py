...
Retrieval is unchanged for now: ``read_worldview()`` still returns the markdown
file at ``settings.worldview_path``. That file is **no longer written** by this
module, so it is effectively frozen until retrieval is migrated to Graphiti
search in a later step. Readers: responder_node (app/services/llm.py).
"""
...
async def read_worldview(query: str | None = None) -> str:
    """Retrieve relevant world-view facts from Graphiti for prompt injection.

    ``query`` is the search text (currently the latest human message in the
    buffer — see responder_node in app/services/llm.py). Uses Graphiti's advanced
    ``search_`` with the COMBINED_HYBRID_SEARCH_RRF recipe, but we harvest only
    two of the layers it returns:
    ...
    Returns the matches as a ``- ``-bulleted markdown block (edge facts first,
    then episode bodies), de-duplicated, or "" when there's no query, no match, or
    on any error — retrieval must never crash the responder.
    """
