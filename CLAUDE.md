...
- **`consolidation_node`**: fanned out via LangGraph `Send`, one instance per user with new facts. Each reads that user's stored profile, merges the new facts with `USER_FACT_CONSOLIDATION_SYSTEM_PROMPT` (de-dup + conflict resolution, newer wins), and writes it back. A per-user `asyncio.Lock` (`_user_locks`, single-process only) guards the read-modify-write so concurrent finalizations touching the same user can't clobber each other.

`update_user_facts(conversation, summary)` is the entry point and never raises. Profiles are read via `get_user_facts` / written via `set_user_facts` (UPSERT) in the repository; `responder_node` reads them in batch via `get_user_facts_batch`, cached per user_id for `USER_FACTS_CACHE_TTL = 5 min` in `_user_facts_cache` (empty results are cached too, to avoid re-querying users with no facts).
